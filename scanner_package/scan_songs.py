...
# Cloudflare sync (used by --sync). Read from env so secrets aren't hardcoded.
...
def sync_to_cloudflare():
    """POST songs_export.json to the worker's /api/import (upsert, non-destructive)."""
    import urllib.request

    if not WEBHOOK_SECRET:
        print("SKIP sync: WEBHOOK_SECRET not set (export WORKER_URL and WEBHOOK_SECRET env vars).")
        return False

    songs = export_json()
    if not songs:
        print("SKIP sync: no done songs to upload.")
        return False
...
async def main():
    ...
    parser.add_argument("--sync", action="store_true",
                        help="after scanning, POST songs_export.json to Cloudflare /api/import")
